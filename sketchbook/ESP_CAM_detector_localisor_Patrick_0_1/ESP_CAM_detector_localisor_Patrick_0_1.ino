...
// 
  ...
  // prise d'une localisation  au debut
      Serial.println("+++ prise une localisation au debut ");
      capture_still();
      Serial.println("+++ prise une localisation au debut fin ");      
    /*  
    if (!capture_still()) {
        Serial.println("Failed capture");
        delay(3000);
...
// on ne fait pas    
// enregistrement de la photo sur la carte SD 
/*
 file_number = file_number + 1;

 //char *filename = (char*)malloc(21 + sizeof(file_number));
 // sprintf(filename, "/sdcard/capture_%d.jpg", file_number);
  String path = "/sd_card" + String(file_number) +".jpg";
  Serial.printf("Picture file name: %s\n", path.c_str());
  
  FILE *file = fopen(path.c_str(), "w");
  if (file!= NULL)  {
    size_t err = fwrite(frame_buffer->buf, 1, frame_buffer->len, file);
    Serial.printf("File saved: %s\n", path.c_str());
  }  else  {
    Serial.println("Could not open file");
  }
...
/*
 ...
 // fs::FS &fs = SD_MMC; 
  ...
  if(!file2){
    ...
    file2.write(frame_buffer->buf, frame_buffer->len); // payload (image), payload length
